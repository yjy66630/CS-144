...
template<typename AdaptT> class TCPSpongeSocket : public LocalStreamSocket
{
private:
    ...
    //! eventloop that handles all the events (new inbound datagram, new outbound bytes, new inbound
    //! bytes)
...
public:
    //! Construct from the interface that the TCPConnection thread will use to read and write
    //! datagrams
    ...
    //! \name
    //! This object cannot be safely moved or copied, since it is in use by two threads
    //! simultaneously
    ...
    //! \name
    //! Some methods of the parent Socket wouldn't work as expected on the TCP socket, so delete
    //! them
...
//! - listen_and_accept() is a blocking function call that acts as both [listen(2)](\ref
//! man2::listen)
...
//! Helper class that makes a TCPOverIPv4overEthernetSpongeSocket behave more like a (kernel)
//! TCPSocket
