...
void
TCPSpongeSocket<AdaptT>::_tcp_loop(const function<bool()>& condition)
{
...
template<typename AdaptT>
void
TCPSpongeSocket<AdaptT>::_initialize_TCP(const TCPConfig& config)
{
    _tcp.emplace(config);
...
//! \brief Call [socketpair](\ref man2::socketpair) and return connected Unix-domain sockets of
//! specified type \param[in] type is the type of AF_UNIX sockets to create (e.g., SOCK_SEQPACKET)
//! \returns a std::pair of connected sockets
static inline pair<FileDescriptor, FileDescriptor>
socket_pair_helper(const int type)
{
    int fds[2];
...
template<typename AdaptT> TCPSpongeSocket<AdaptT>::~TCPSpongeSocket()
{
    try {
...
template<typename AdaptT>
void
TCPSpongeSocket<AdaptT>::wait_until_closed()
{
    shutdown(SHUT_RDWR);
    if (_tcp_thread.joinable()) {
...
template<typename AdaptT>
void
TCPSpongeSocket<AdaptT>::connect(const TCPConfig& c_tcp, const FdAdapterConfig& c_ad)
{
    if (_tcp) {
...
template<typename AdaptT>
void
TCPSpongeSocket<AdaptT>::listen_and_accept(const TCPConfig& c_tcp, const FdAdapterConfig& c_ad)
{
    if (_tcp) {
...
template<typename AdaptT>
void
TCPSpongeSocket<AdaptT>::_tcp_main()
{
    try {
...
void
CS144TCPSocket::connect(const Address& address)
{
...
FullStackSocket::FullStackSocket() :
    TCPOverIPv4OverEthernetSpongeSocket(TCPOverIPv4OverEthernetAdapter(
...
void
FullStackSocket::connect(const Address& address)
{
    TCPConfig tcp_config;
    tcp_config.rt_timeout = 100;
