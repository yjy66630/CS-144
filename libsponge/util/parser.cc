...
void
NetParser::_check_size(const size_t size)
{
...
void
NetParser::remove_prefix(const size_t n)
{
    _check_size(n);
    if (error()) {
        return;
...
template<typename T>
void
NetUnparser::_unparse_int(string& s, T val)
{
    constexpr size_t len = sizeof(T);
    for (size_t i = 0; i < len; ++i) {
...
NetParser::u32()
{
    return _parse_int<uint32_t>();
}

uint16_t
NetParser::u16()
{
    return _parse_int<uint16_t>();
}

uint8_t
NetParser::u8()
{
    return _parse_int<uint8_t>();
}

void
NetUnparser::u32(string& s, const uint32_t val)
{
    return _unparse_int<uint32_t>(s, val);
}

void
NetUnparser::u16(string& s, const uint16_t val)
{
    return _unparse_int<uint16_t>(s, val);
}

void
NetUnparser::u8(string& s, const uint8_t val)
{
