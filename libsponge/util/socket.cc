...
Address
Socket::get_address(const string& name_of_function,
...
//! \param[in] how can be `SHDN_RD`, `SHDN_WR`, or `SHDN_RDWR`; see [shutdown(2)](\ref
//! man2::shutdown)
void
Socket::shutdown(const int how)
{
    ...
    switch (how) {
        case SHUT_RD: register_read(); break;
        case SHUT_WR: register_write(); break;
        case SHUT_RDWR:
            register_read();
            register_write();
            break;
        default: throw runtime_error("Socket::shutdown() called with invalid `how`");
    }
}
...
//! std::runtime_error
void
UDPSocket::recv(received_datagram& datagram, const size_t mtu)
{
...
UDPSocket::recv(const size_t mtu)
{
...
void
sendmsg_helper(const int fd_num, const sockaddr* destination_address,
...
void
UDPSocket::sendto(const Address& destination, const BufferViewList& payload)
{
...
void
UDPSocket::send(const BufferViewList& payload)
{
...
//! man2::listen))
void
TCPSocket::listen(const int backlog)
{
...
void
Socket::setsockopt(const int level, const int option, const option_type& option_value)
{
