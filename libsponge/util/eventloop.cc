...
unsigned int
EventLoop::Rule::service_count() const
{
    return direction == Direction::In ? fd.read_count() : fd.write_count();
}

//! \param[in] fd is the FileDescriptor to be polled
//! \param[in] direction indicates whether to poll for reading (Direction::In) or writing
//! (Direction::Out) \param[in] callback is called when `fd` is ready. \param[in] interest is called
//! by EventLoop::wait_next_event. If it returns `true`, `fd` will
//!                     be polled, otherwise `fd` will be ignored only for this execution of
//!                     `wait_next_event.
//! \param[in] cancel is called when the rule is cancelled (e.g. on hangup, EOF, or closure).
void
EventLoop::add_rule(const FileDescriptor& fd, const Direction direction, const CallbackT& callback,
                    const InterestT& interest, const CallbackT& cancel)
{
    _rules.push_back({fd.duplicate(), direction, callback, interest, cancel});
}

//! \param[in] timeout_ms is the timeout value passed to [poll(2)](\ref man2::poll);
//! `wait_next_event`
...
//! writability (if Rule::direction == Direction::Out) unless Rule::fd has reached EOF, in which
//! case the Rule is canceled (i.e., deleted from EventLoop::_rules).
//!
...
//!
//! If a [signal(7)](\ref man7::signal) was caught during polling or if EventLoop::_rules becomes
//! empty, this function returns Result::Exit.
//!
//! If a timeout occurred while polling (i.e., no fd became ready), this function returns
//! Result::Timeout.
//!
//! Otherwise, this function returns Result::Success.
//!
//! \b IMPORTANT: every call to Rule::callback must read from or write to Rule::fd, or the
//! `interest` callback must stop returning true after the callback completes. If none of these
//! conditions occur, EventLoop::wait_next_event will throw std::runtime_error. This is because
//! [poll(2)](\ref man2::poll) is level triggered, so failing to act on a ready file descriptor will
//! result in a busy loop (poll returns on a ready file descriptor; file descriptor is not read or
//! written, so it is still ready; the next call to poll will immediately return).
EventLoop::Result
EventLoop::wait_next_event(const int timeout_ms)
{
