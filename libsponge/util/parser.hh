...
private:
    ...
    template<typename T> T _parse_int();
...
public:
    ...
    error() const
    {
        return get_error() != ParseResult::NoError;
    }
...
struct NetUnparser
{
    template<typename T> static void _unparse_int(std::string& s, T val);
