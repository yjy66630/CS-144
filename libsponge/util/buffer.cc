...
void
Buffer::remove_prefix(const size_t n)
{
    if (n > str().size()) {
...
void
BufferList::append(const BufferList& other)
{
...
string
BufferList::concatenate(size_t n) const
{
    std::string ret;
    ret.reserve(n);
    for (const auto& buf : _buffers) {
...
void
BufferList::remove_prefix(size_t n)
{
    while (n > 0) {
        if (_buffers.empty()) {
...
void
BufferViewList::remove_prefix(size_t n)
{
    while (n > 0) {
        if (_views.empty()) {
...
vector<iovec>
BufferViewList::as_iovecs() const
{
    vector<iovec> ret;
