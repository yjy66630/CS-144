...
// in LISTEN, send ACK
static void
ack_listen_test(const TCPConfig& cfg, const WrappingInt32 seqno, const WrappingInt32 ackno,
                const int lineno)
{
    try {
...
static void
ack_rst_syn_sent_test(const TCPConfig& cfg, const WrappingInt32 base_seq, const WrappingInt32 seqno,
                      const WrappingInt32 ackno, const bool send_rst, const int lineno)
{
    try {
