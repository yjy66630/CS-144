...
struct ReceiverTestStep
{
    ...
    to_string() const
    {
        return "ReceiverTestStep";
    }
    virtual void
    execute(TCPReceiver&) const
    {
    }
...
struct ExpectState : public ReceiverExpectation
{
    ...
    description() const
    {
        return "in state `" + _state + "`";
    }
    void
    execute(TCPReceiver& receiver) const
    {
...
struct ExpectWindow : public ReceiverExpectation
{
    ...
    description() const
    {
        return "window " + std::to_string(_window);
    }
...
struct ExpectUnassembledBytes : public ReceiverExpectation
{
    ...
    description() const
    {
        return std::to_string(_n_bytes) + " unassembled bytes";
    }
...
struct SegmentArrives : public ReceiverAction
{
    ...
    static std::string
    result_name(Result res)
    {
        switch (res) {
            case Result::NOT_SYN: return "(no SYN received)";
            case Result::OUT_OF_WINDOW: return "false (segment does not overlap the window)";
            case Result::OK: return "true (segment is at least partly in the window)";
    ...
    SegmentArrives&
    with_ack(uint32_t ackno_)
    {
        return with_ack(WrappingInt32{ackno_});
    }
    ...
    SegmentArrives&
    with_seqno(WrappingInt32 seqno_)
    {
        seqno = seqno_;
        return *this;
    }
    ...
    SegmentArrives&
    with_seqno(uint32_t seqno_)
    {
        return with_seqno(WrappingInt32{seqno_});
    }

    SegmentArrives&
    with_win(uint16_t win_)
    {
        win = win_;
        return *this;
    }
    ...
    void
    execute(TCPReceiver& receiver) const override
    {
        TCPSegment seg = build_segment();
        std::ostringstream o;
        o << seg.header().summary();
...
public:
    ...
    void
    execute(const ReceiverTestStep& step)
    {
        try {
