...
NetworkInterfaceTestStep::operator std::string() const
{
    return "NetworkInterfaceTestStep";
}

void
NetworkInterfaceTestStep::execute(NetworkInterface&) const
{
}
...
template<typename T>
NetworkInterfaceExpectationViolation
NetworkInterfaceExpectationViolation::property(const std::string& property_name, const T& expected,
                                               const T& actual)
{
...
std::string
NetworkInterfaceExpectation::description() const
{
    return "description missing";
}

void
NetworkInterfaceExpectation::execute(NetworkInterface&) const
{
}
...
NetworkInterfaceAction::operator std::string() const
{
    return "     Action: " + description();
}

std::string
NetworkInterfaceAction::description() const
{
    return "description missing";
}

void
NetworkInterfaceAction::execute(NetworkInterface&) const
{
}
...
string
summary(const EthernetFrame& frame)
{
    ...
    switch (frame.header().type) {
        ...
        default: out.append("unknown frame type"); break;
    }
    return out;
}

void
NetworkInterfaceTestHarness::execute(const NetworkInterfaceTestStep& step)
{
    try {
...
string
SendDatagram::description() const
{
...
void
SendDatagram::execute(NetworkInterface& interface) const
{
    interface.send_datagram(dgram, next_hop);
}

string
ReceiveFrame::description() const
{
    return "frame arrives (" + summary(frame) + ")";
}

void
ReceiveFrame::execute(NetworkInterface& interface) const
{
...
string
ExpectFrame::description() const
{
    return "frame transmitted (" + summary(expected) + ")";
}

void
ExpectFrame::execute(NetworkInterface& interface) const
{
...
string
ExpectNoFrame::description() const
{
    return "no frame transmitted";
}

void
ExpectNoFrame::execute(NetworkInterface& interface) const
{
...
string
Tick::description() const
{
    return to_string(_ms) + " ms pass";
}

void
Tick::execute(NetworkInterface& interface) const
{
    interface.tick(_ms);
}
