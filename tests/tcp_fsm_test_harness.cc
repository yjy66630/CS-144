...
void
TestFD::write(const BufferViewList& buffer)
{
...
void
TestFdAdapter::config_segment(TCPSegment& seg)
{
...
void
TestFdAdapter::write(TCPSegment& seg)
{
    config_segment(seg);
...
//! \param[in] ackno is the optional acknowledgment number of the segment; if no value, ACK flag is
//! not set
void
TCPTestHarness::send_fin(const WrappingInt32 seqno, const optional<WrappingInt32> ackno)
{
    SendSegment step{};
    if (ackno.has_value()) {
...
//! \param[in] swin is the optional window size for the segment; if no value, uses default value
//! (137 bytes)
void
TCPTestHarness::send_ack(const WrappingInt32 seqno, const WrappingInt32 ackno,
...
//! not set
void
TCPTestHarness::send_rst(const WrappingInt32 seqno, const optional<WrappingInt32> ackno)
{
    SendSegment step{};
    if (ackno.has_value()) {
        step.with_ack(true).with_ackno(ackno.value());
...
//! not set
void
TCPTestHarness::send_syn(const WrappingInt32 seqno, const optional<WrappingInt32> ackno)
{
    SendSegment step{};
    if (ackno.has_value()) {
        step.with_ack(true).with_ackno(ackno.value());
...
//! \param[in] seqno is the sequence number of the segment
//! \param[in] ackno is the optional acknowledgment number of the segment; if no value, ACK flag is
//! not set \param[in] val is the value of the one-byte payload
void
TCPTestHarness::send_byte(const WrappingInt32 seqno, const optional<WrappingInt32> ackno,
                          const uint8_t val)
{
    SendSegment step{};
    if (ackno.has_value()) {
...
void
TCPTestHarness::send_data(const WrappingInt32 seqno, const WrappingInt32 ackno, VecIterT begin,
                          VecIterT end)
{
    SendSegment step{};
    execute(SendSegment{}
...
void
TCPTestHarness::execute(const TCPTestStep& step, std::string note)
{
    try {
        step.execute(*this);
...
TCPSegment
TCPTestHarness::expect_seg(const ExpectSegment& expectation, std::string note)
{
    try {
...
TCPTestHarness
TCPTestHarness::in_listen(const TCPConfig& cfg)
{
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_syn_sent(const TCPConfig& cfg, const WrappingInt32 tx_isn)
{
    TCPConfig c{cfg};
    c.fixed_isn = tx_isn;
    TCPTestHarness h{c};
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_established(const TCPConfig& cfg, const WrappingInt32 tx_isn,
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_close_wait(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                              const WrappingInt32 rx_isn)
{
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_last_ack(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                            const WrappingInt32 rx_isn)
{
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_fin_wait_1(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                              const WrappingInt32 rx_isn)
{
    TCPTestHarness h = in_established(cfg, tx_isn, rx_isn);
    h.execute(Close{});
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_fin_wait_2(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                              const WrappingInt32 rx_isn)
{
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_closing(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                           const WrappingInt32 rx_isn)
{
    TCPTestHarness h = in_fin_wait_1(cfg, tx_isn, rx_isn);
    h.send_fin(rx_isn + 1, tx_isn + 1);
...
//!            seqno for the SYN.
TCPTestHarness
TCPTestHarness::in_time_wait(const TCPConfig& cfg, const WrappingInt32 tx_isn,
                             const WrappingInt32 rx_isn)
{
    TCPTestHarness h = in_fin_wait_1(cfg, tx_isn, rx_isn);
