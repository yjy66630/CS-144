...
//! \brief A wrapper class for a SOCK_SEQPACKET [Unix-domain socket](\ref man7::unix), for use by
//! TCPTestHarness \details A TestFD comprises the two endpoints of a Unix-domain socket connection.
...
public:
    ...
    void send_byte(const WrappingInt32 seqno, const std::optional<WrappingInt32> ackno,
                   const uint8_t val);

    //! construct a segment containing the specified payload and inject it into TCPConnection
    void send_data(const WrappingInt32 seqno, const WrappingInt32 ackno, VecIterT begin,
                   VecIterT end);

    //! is it possible to read from the TestFdAdapter (i.e., read a segment TCPConnection previously
    //! wrote)?
    bool
    can_read() const
    {
        return _flt.can_read();
    }
    ...
    //! \details SYNs have been traded, then the machine received and ACK'd FIN, and then it sent
    //! its own FIN.
    //!          No payload was exchanged.
    //! \param[in] tx_isn is the ISN of the FSM's outbound sequence. i.e. the
    //!            seqno for the SYN.
