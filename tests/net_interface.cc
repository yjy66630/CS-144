...
InternetDatagram
make_datagram(const string& src_ip, const string& dst_ip)
{
...
ARPMessage
make_arp(const uint16_t opcode, const EthernetAddress sender_ethernet_address,
...
EthernetFrame
make_frame(const EthernetAddress& src, const EthernetAddress& dst, const uint16_t type,
    ...
    try {
        {
            ...
            NetworkInterfaceTestHarness test{
                "typical ARP workflow", local_eth, Address("4.3.2.1", 0)};
            ...
            test.execute(ExpectFrame{make_frame(
                ...
                make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "4.3.2.1", {}, "192.168.0.1")
                    .serialize())});
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, target_eth, EthernetHeader::TYPE_IPv4, datagram.serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ReceiveFrame(
                make_frame(
                    target_eth, local_eth, EthernetHeader::TYPE_IPv4, reply_datagram.serialize()),
                reply_datagram));
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ReceiveFrame(
                make_frame(
                    target_eth, another_eth, EthernetHeader::TYPE_IPv4, reply_datagram.serialize()),
                {}));
        }

        {
            ...
            NetworkInterfaceTestHarness test{
                "reply to ARP request", local_eth, Address("5.5.5.5", 0)};
            test.execute(ReceiveFrame{
                make_frame(
                    remote_eth,
                    ETHERNET_BROADCAST,
                    EthernetHeader::TYPE_ARP,
                    make_arp(ARPMessage::OPCODE_REQUEST, remote_eth, "10.0.1.1", {}, "7.7.7.7")
                        .serialize()),
                {}});
            test.execute(ExpectNoFrame{});
            test.execute(ReceiveFrame{
                make_frame(
                    ...
                    ETHERNET_BROADCAST,
                    EthernetHeader::TYPE_ARP,
                    make_arp(ARPMessage::OPCODE_REQUEST, remote_eth, "10.0.1.1", {}, "5.5.5.5")
                        .serialize()),
                {}});
            test.execute(ExpectFrame{make_frame(
                local_eth,
                remote_eth,
                EthernetHeader::TYPE_ARP,
                make_arp(ARPMessage::OPCODE_REPLY, local_eth, "5.5.5.5", remote_eth, "10.0.1.1")
                    .serialize())});
            test.execute(ExpectNoFrame{});
        }

        {
            ...
            const EthernetAddress remote_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{
                "learn from ARP request", local_eth, Address("5.5.5.5", 0)};
            test.execute(ReceiveFrame{
                make_frame(
                    remote_eth,
                    ETHERNET_BROADCAST,
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, remote_eth, EthernetHeader::TYPE_IPv4, datagram.serialize())});
            test.execute(ExpectNoFrame{});
        }

        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{
                "pending mappings last five seconds", local_eth, Address("1.2.3.4", 0)};

            test.execute(
                SendDatagram{make_datagram("5.6.7.8", "13.12.11.10"), Address("10.0.0.1", 0)});
            test.execute(ExpectFrame{make_frame(
                local_eth,
                ETHERNET_BROADCAST,
                EthernetHeader::TYPE_ARP,
                make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "1.2.3.4", {}, "10.0.0.1")
                    .serialize())});
            test.execute(ExpectNoFrame{});
            test.execute(Tick{4990});
            test.execute(
                SendDatagram{make_datagram("17.17.17.17", "18.18.18.18"), Address("10.0.0.1", 0)});
            test.execute(ExpectNoFrame{});
            test.execute(Tick{20});
            // pending mapping should now expire
            test.execute(
                SendDatagram{make_datagram("42.41.40.39", "13.12.11.10"), Address("10.0.0.1", 0)});
            test.execute(ExpectFrame{make_frame(
                local_eth,
                ETHERNET_BROADCAST,
                EthernetHeader::TYPE_ARP,
...

        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{
                "active mappings last 30 seconds", local_eth, Address("4.3.2.1", 0)};
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, target_eth, EthernetHeader::TYPE_IPv4, datagram2.serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, target_eth, EthernetHeader::TYPE_IPv4, datagram3.serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ReceiveFrame{make_frame(new_target_eth,
                                                 local_eth,
                                                 EthernetHeader::TYPE_ARP,
                                                 make_arp(ARPMessage::OPCODE_REPLY,
                                                          new_target_eth,
                                                          "192.168.0.1",
                                                          local_eth,
                                                          "4.3.2.1")
                                      ...
                                      {}});
            test.execute(ExpectFrame{make_frame(
                local_eth, new_target_eth, EthernetHeader::TYPE_IPv4, datagram4.serialize())});
            test.execute(ExpectNoFrame{});
        }

        {
            ...
            test.execute(ReceiveFrame{
                make_frame(
                    ...
                    ETHERNET_BROADCAST,
                    EthernetHeader::TYPE_ARP,
                    make_arp(ARPMessage::OPCODE_REQUEST, remote_eth1, "10.0.0.5", {}, "10.0.0.1")
                        .serialize()),
                {}});
            test.execute(ExpectFrame{make_frame(
                local_eth,
                remote_eth1,
                EthernetHeader::TYPE_ARP,
                make_arp(ARPMessage::OPCODE_REPLY, local_eth, "10.0.0.1", remote_eth1, "10.0.0.5")
                    .serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ReceiveFrame{
                make_frame(
                    ...
                    ETHERNET_BROADCAST,
                    EthernetHeader::TYPE_ARP,
                    make_arp(ARPMessage::OPCODE_REQUEST, remote_eth2, "10.0.0.19", {}, "10.0.0.1")
                        .serialize()),
                {}});
            test.execute(ExpectFrame{make_frame(
                local_eth,
                remote_eth2,
                EthernetHeader::TYPE_ARP,
                make_arp(ARPMessage::OPCODE_REPLY, local_eth, "10.0.0.1", remote_eth2, "10.0.0.19")
                    .serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, remote_eth1, EthernetHeader::TYPE_IPv4, datagram.serialize())});
            test.execute(ExpectFrame{make_frame(
                local_eth, remote_eth2, EthernetHeader::TYPE_IPv4, datagram2.serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{make_frame(
                local_eth, remote_eth2, EthernetHeader::TYPE_IPv4, datagram3.serialize())});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{make_frame(
                ...
                ETHERNET_BROADCAST,
                EthernetHeader::TYPE_ARP,
                make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "10.0.0.1", {}, "10.0.0.5")
                    .serialize())});
            test.execute(ExpectNoFrame{});
        }
