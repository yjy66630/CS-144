...
void
bidirectional_stream_copy(Socket& socket)
{
    ...
    _eventloop.add_rule(
        ...
        [&] {
            return (not _outbound.error()) and (_outbound.remaining_capacity() > 0) and
                   (not _inbound.error());
        },
        [&] { _outbound.end_input(); });
    ...
    _eventloop.add_rule(
        ...
        [&] {
            return (not _inbound.error()) and (_inbound.remaining_capacity() > 0) and
                   (not _outbound.error());
        },
        [&] { _inbound.end_input(); });
